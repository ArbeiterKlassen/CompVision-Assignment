...
CMenuEx::CMenuEx()
{
	...
	m_strFontFaceName = _T("黑体");
	m_nLeftWidth = 30;
	m_nIconWidth = 16;
	m_nIconHeight = 16;
	m_clrBk = RGB(57, 63, 73);
	m_clrBkHover = RGB(19, 68, 93);
...
void CMenuEx::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	else
	{
		...
		lstrcpy(fontInfo.lfFaceName, m_strFontFaceName);
		CFont fontCh;
...
LRESULT WINAPI CallWndProc(int code, WPARAM wParam, LPARAM lParam)
{
	...
	while (code == HC_ACTION)//HC_ACTION 为必须处理  
	{
		...
		WNDPROC lastWndProc = (WNDPROC)GetWindowLongPtr(hWnd, GWL_WNDPROC); //获得指定窗口的信息 GWL_WNDPROC得到窗口回调函数的地址或句柄  
...
void CMenuEx::SetFontInfo(int nSize, const char* szFontFaceName/*=NULL*/)
{
	...
	{
		m_strFontFaceName = (LPCTSTR)szFontFaceName;
	}
}
